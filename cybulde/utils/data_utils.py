...
def get_cmd_to_get_raw_data(
    ...
    """Get shell command to download the raw data from the dvc store
    ...
    ----------
    str
        shell command to download the raw data from the dvc store
    """
...
def get_nrof_partitions(df_memory_usage: int,
                        nrof_workers: int, 
                        available_memory: Optional[float],
                        min_partition_size: int, 
                        aimed_nrof_partitions_per_worker: int) -> int:
...
def repartition_dataframe(
    ...
    aimed_nrof_partitions_per_worker: int = 10) -> dd.core.DataFrame:
